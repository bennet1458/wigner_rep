...
# k_1 = 2*np.arccos((n-2*p)/(2*np.emath.sqrt(p*(n-p))))
# k_2 = -2*np.arccos((n-2*p)/(2*np.emath.sqrt(p*(n-p))))

arg = -(n - 2*p)/(2*np.emath.sqrt(p*(n-p)))
k_1 = 2*np.emath.arccos(arg)
...
def factor():
    return np.emath.sqrt(1/(2*np.pi*n))+p*0
...
print(np.unique(np.sign(thetadd_plus(k_1).real)))
print(np.unique(np.sign(thetadd_plus(k_2).real)))
print(1/np.emath.sqrt(thetadd_plus(k_1))[0])
print(1/np.emath.sqrt(thetadd_plus(k_1))[2])
print(1/np.emath.sqrt(thetadd_plus(k_2))[0])
print(1/np.emath.sqrt(thetadd_plus(k_2))[2])

print(np.min(k_1), np.max(k_1))
print(np.min(k_2), np.max(k_2))


def theta_prime_plus(k):
    return -0.5 - np.cos(k/2)/(2*np.emath.sqrt(2 - np.sin(k/2)**2))

def theta_prime_minus(k):
    return -0.5 + np.cos(k/2)/(2*np.emath.sqrt(2 - np.sin(k/2)**2))

print(n*theta_prime_plus(k_1) + p)
print(n*theta_prime_plus(k_2) + p)
print(n*theta_prime_minus(k_1) + p)
print(n*theta_prime_minus(k_2) + p)

print(k_1[:10])
print(k_1[-10:])

test = np.cos(k_1/2)/np.sqrt(2 - np.sin(k_1/2)**2)

print(test)

lhs = np.cos(k_1/2)/np.sqrt(2 - np.sin(k_1/2)**2)
rhs = 2*p/n - 1
