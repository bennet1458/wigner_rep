...
t_2 = 50e-6 #150.0e-6   # time after second kick
m = 1.4e-19   # mass
...
###################
p = np.linspace(-4*sigma_p, 4*sigma_p, res_q)
x = np.linspace(-4*sigma_x-d-2*sigma_p*t_2/m, 4*sigma_x+d+2*sigma_p*t_2/m, res_x)
...
axes[0, 0].set_title('After double slit ($t_0$)')
...
axes[0, 1].set_title("After time evolution ($t_1$)")
