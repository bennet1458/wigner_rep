...
# grid
res = 1000
# x = np.linspace(-4*sigma_x, 4*sigma_x+(q+2*sigma_p)*t_1/m, res)
x = np.linspace(-8*sigma_x+(q/2-2*sigma_p)*t_1/m, 8*sigma_x+(q/2+2*sigma_p)*t_1/m, res)
p = np.linspace(-4*sigma_p+q/2, 4*sigma_p+q/2, res)
X = x[:, np.newaxis]
P = p[np.newaxis, :]
...
W_dc1_t1_k1_marginal = integrate_over_p(W_dc1_t1_k1)

# Subplot 3: Wigner function after second kick
im3 = axes[1, 0].plot(x, W_t1_k1_marginal, linewidth=2)
axes[1, 0].set_xlabel("x")
axes[1, 0].set_ylabel("Marginal")


# Subplot 4: Wigner function after second kick and time evolution
im4 = axes[1, 1].plot(x, W_dc1_t1_k1_marginal, linewidth=2)
