...
sep = 1 # separation of the two wave packets after the kick, in units of sigma_x
...
def q_function(T, hbar=hbar, m=1, omega=1, k_B=k_B):
    q = sep*np.sqrt(hbar*m*omega/3)*np.sqrt(2/(np.exp((hbar*omega)/(k_B*T))-1)+1)
    return q

T_values = np.logspace(-8, 1, 500)
...
plt.ylabel('Momentum distribution $\sigma_p$ [kg*m/s]', fontsize=12)
plt.title('Momentum distribution of thermal state', fontsize=14)
# plt.title(f'Minimum kick strength needed for wave package seperation.', fontsize=14)
