...
for k_n in k_ns:
    print('...............')
    print(np.shape(factor()[np.newaxis, :]))
    print(np.shape(1/np.emath.sqrt(thetadd_plus(k_n)[np.newaxis, :])))
    print(np.shape(exp_arg_plus(k_n)[np.newaxis, :]))
    print(np.shape(np.sum(pi_plus(k_n)*psi_0[np.newaxis, :, np.newaxis], axis=1)))
    print(np.shape(factor()[np.newaxis, :]
