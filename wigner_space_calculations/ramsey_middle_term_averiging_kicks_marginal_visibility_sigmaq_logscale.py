...
marg = p_marginal(q, q)
amp_marg = (np.max(marg) - np.min(marg))/2
...
coordinate = np.log(amp_marg_av/amp_marg)
# Subplot 2: Wigner function after second kick
plt.plot(sigma_q_values, coordinate, linewidth=2,  color='blue', label='Numerically determined values')
...
popt, pcov = curve_fit(func, sigma_q_values, coordinate, p0=[3e23])
popt2, pcov2 = curve_fit(func2, sigma_q_values, coordinate, p0=[3e23])
popt3, pcov3 = curve_fit(funcsqrt, sigma_q_values, coordinate, p0=[3e23])
print(popt)
...
plt.plot(sigma_q_values, funcsqrt(sigma_q_values, *popt3), 'y-', label='fit exp(-b*$\sqrt{\sigma_q}$)')
# plt.hlines(y=np.log(np.exp(-1)), xmin=0, xmax=sigma_q_end, color='black', linestyle='--', label='1/e')
plt.vlines(x=sigma_p, ymin=np.min(func2(sigma_q_values, *popt2)), ymax=np.max(func2(sigma_q_values, *popt2)), color='black', linestyle='--', label='$\sigma_p$')
plt.vlines(x=1/np.sqrt(q**2*t_1**2/(4*hbar**2*m**2)), ymin=np.min(func2(sigma_q_values, *popt2)), ymax=np.max(func2(sigma_q_values, *popt2)), color='black', linestyle=':', label='calculated $\sigma_{q, max}=1/\sqrt{C}$')
plt.grid()
plt.xlabel("Kick distribution width $\sigma_q$")
plt.ylabel("Visibility of the Ramsey fringes in logarithmic scale")
# plt.ylim(-5, 0)
plt.legend()
